...
public class FtpUtil {
     ...
     * @throws IOException
     */
    public boolean upLoadByFtp(String absSrcFileName, String destDir, String destFileName, FTPClient ftpClient) throws IOException {
        boolean success = false;
        // 创建并转到工作目录
        ...
        if (files.length == 1) {// 断点续传
            ...
            success = uploadFile(destFileName, srcFile, dstFileSize, ftpClient);
            if (!success) {// 如果断点续传没有成功，则删除服务器上文件，重新上传
                if (ftpClient.deleteFile(destFileName)) {
                    success = uploadFile(destFileName, srcFile, 0, ftpClient);
                } else {
     ...
     * @throws IOException
     */
    public boolean downLoadByFtp(String remoteFileName, String localFileName, FTPClient ftpClient) throws IOException {
