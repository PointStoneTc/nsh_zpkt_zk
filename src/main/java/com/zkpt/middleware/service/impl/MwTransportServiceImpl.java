...
public class MwTransportServiceImpl implements IMwTransportService {
    ...
    @Autowired
    private IBankUnUsefulService bankUnUsefulService;
    ...
    public boolean trans_in(BankProtocalPackage bankProtocalPackage) {
        try {
            // 更改银行编码
            String convertBankCode = env.getProperty("middleware.bank.newCode");
            ...
            case GENERAL_LEDGER:
                ...
                break;
            default:
                bankUnUsefulService.request(bankProtocalPackage);
                break;
            }
        } catch (Exception e) {
            e.printStackTrace();
