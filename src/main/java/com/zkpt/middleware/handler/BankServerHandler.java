...
public class BankServerHandler extends IoHandlerAdapter {
    ...
    private Map<String, IoSession> sessions_ip = new HashMap<String, IoSession>(); // 控制某个IP的连接
    private int sessionSize = 0; // 控制连接总量
    private int maxSessionSize;
    private boolean lockedIp;
    ...
    public void sessionOpened(IoSession session) throws Exception {
        maxSessionSize = Integer.parseInt(env.getProperty("middleware.maxSessionSize"));
        lockedIp = Boolean.parseBoolean(env.getProperty("middleware.lockedIp"));
        ...
        String ip = address.getHostAddress();
        int port = inetSocketAddress.getPort();
        if (lockedIp && null != sessions_ip.get(ip)) {
